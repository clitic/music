...
const THEME_ICONS = { adaptive: SunMoon, light: Sun, dark: Moon } as const;

export function Header({ theme, onCycleTheme, lastUpdated }: HeaderProps) {
  const ThemeIcon = THEME_ICONS[theme];

  return (
    <header className="flex items-center gap-3 max-md:gap-2 px-4 max-md:px-3 py-2 bg-bg-primary border-b border-divider sticky top-0 z-50 min-h-14 max-md:min-h-12">
      <div className="flex items-center gap-1 whitespace-nowrap shrink-0">
        <svg xmlns="http://www.w3.org/2000/svg" width="93" height="20" viewBox="0 0 93 20" focusable="false" aria-hidden="true" className="shrink-0 max-md:w-[75px] max-md:h-[16px]">
          <g>
        ...
        <span className="text-xl max-md:text-base font-normal text-text-secondary tracking-tight opacity-80 leading-none translate-y-[1.5px]">Trending Music</span>
        {lastUpdated && (
          <span className="text-[0.7rem] font-medium text-text-secondary bg-bg-secondary px-2.5 py-0.5 rounded-full translate-y-[1.5px]">
            {lastUpdated}
          </span>
        )}
      </div>

      <div className="flex items-center gap-2 max-md:gap-1 ml-auto">
        <a
          href="https://github.com/clitic/music"
          target="_blank"
          rel="noopener"
          className="group relative w-10 h-10 max-md:w-9 max-md:h-9 rounded-full flex items-center justify-center hover:bg-bg-hover transition-colors shrink-0"
          aria-label="Source Code"
        >
          <GithubIcon size={20} />
          <span className="absolute -bottom-7.5 left-1/2 -translate-x-1/2 bg-text-primary text-bg-primary text-xs px-2 py-1 rounded whitespace-nowrap opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity z-50">
            Source Code
          </span>
        </a>
